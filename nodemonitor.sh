...
#### CONFIGURATION #######################################################
config="/home/cosmos/.gaiad/config/config.toml" # config.toml file for node
...
checkpersistentpeers=1 # if equals 1 the number of disconnected persistent peers is checked (when persistent peers are configured in config.toml)
...
if [ -z $validatoraddress ]; then echo "WARNING: lcd appears to be down, start script again when data can be obtained"; exit 1; fi
